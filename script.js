    ...
    if (addDepBtn && depInput) {
        addDepBtn.addEventListener('click', () => {
            ...
            if (val > 0) {
                ...
                updateProgressVisual();
                depInput.value = '';
                showNotification(`Agregado $${val}`);
            }
        });
    }

    // REINICIAR RUTA (CORREGIDO)
    if (resetBtn) {
...
function setupModals() {
    ...
    const closeMap = ['closeFriendsModal', 'closeAddFriendModal', 'closeAchievementModal', 'closeBenefitModal', 'closeEditPersonalRoadmapModal', 'closeEditTeamRoadmapModal', 'closeAvatarModal', 'closeRedemptionModal'];
